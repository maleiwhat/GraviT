...
namespace GVT {
    ...
    namespace Domain {
        ...
        OptixDomain::OptixDomain(const std::string& filename) : GeometryDomain(filename) {
        }

        OptixDomain::~OptixDomain() {
        }

        OptixDomain::OptixDomain(const std::string& filename, GVT::Math::AffineTransformMatrix<float> m) : GVT::Domain::GeometryDomain(filename, m) {
            this->load();
        }

        bool OptixDomain::load() {

            //GVT_ASSERT(!domainIsLoaded(), "Domain already loaded");
