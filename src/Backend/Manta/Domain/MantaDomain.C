...
MantaDomain::MantaDomain(string filename,
                         GVT::Math::AffineTransformMatrix<float> m)
    : GVT::Domain::GeometryDomain(filename, m) {

  if (domainIsLoaded()) {
    Manta::Mesh* mesh =
        GVT::Data::transform<GVT::Data::Mesh*, Manta::Mesh*>(this->mesh);
    ...
    ambient = new Manta::AmbientOcclusionBackground(Manta::Color::white() * 0.5,
                                                    1, 36);
    Manta::Vector lightPosition(-10, 6, -30);
    Manta::PreprocessContext context(rtrt, 0, 1, lights);
    std::cout << "context.global_lights : " << context.globalLights
              << std::endl;
...
struct parallelTrace {
  ...
  parallelTrace(GVT::Domain::MantaDomain* dom, GVT::Data::RayVector& rayList,
                GVT::Data::RayVector& moved_rays, const size_t workSize,
                boost::atomic<size_t>& counter)
      : dom(dom),
        rayList(rayList),
    ...
    while (!rayList.empty()) {
      ...
      while (!localQueue.empty()) {
        ...
        for (int i = 0; i < rayPacket.size(); i++) {
          mRays.setRay(i, GVT::Data::transform<GVT::Data::ray, Manta::Ray>(
                              dom->toLocal(rayPacket[i])));
        }
        ...
        for (int pindex = 0; pindex < rayPacket.size(); pindex++) {
          ...
          if (mRays.wasHit(pindex)) {
            ...
            GVT::Math::Vector4f normal = dom->toWorld(
                GVT::Data::transform<Manta::Vector, GVT::Math::Vector4f>(
            ...
            for (int lindex = 0; lindex < dom->lights.size(); lindex++) {
              ...
              GVT::Data::Color c =
                  dom->mesh->mat->shade(ray, normal, dom->lights[lindex]);
            ...
            if (ndepth > 0 && rayPacket[pindex].w > p) {
              ...
              ray.origin = ray.origin + ray.direction * ray.t;
              ray.setDirection(
                  dom->mesh->mat->CosWeightedRandomHemisphereDirection2(normal)
                      .normalize());
...
void MantaDomain::trace(GVT::Data::RayVector& rayList,
                        GVT::Data::RayVector& moved_rays) {
  ...
  size_t workload = std::max(
      (size_t)1,
      (size_t)(rayList.size() /
               (GVT::Concurrency::asyncExec::instance()->numThreads * 4)));
