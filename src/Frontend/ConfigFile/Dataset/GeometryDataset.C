...
bool ConfigFileDataset<GVT::Domain::GeometryDomain>::init() {
  ...
  GVT_ASSERT(conf_filename.size() > 0,
             "ERROR: Configuration file for dataset not set");
  ...
  while (conf.good()) {
    ...
    if (line.size() > 0) {
      ...
      if (pos == string::npos) {
        cerr << "ERROR: didn't find var number in chunk line: '" << line << "'"
             << endl;
        return false;
      }
      ...
      for (int i = 0; i < 3; ++i) {
        ...
        pos = line.find_first_of(" ", lastpos);
        if (pos == string::npos) {
          cerr << "ERROR: didn't find dim " << i << " in chunk line: '" << line
               << "'" << endl;
          return false;
        }
        buf.str(line.substr(lastpos, pos));
...
//                cerr << "ERROR: invalid domain id '" << id << "' passed to
// GetDomain" << endl;
//                return NULL;
//            }
//
...
//            map< int, GVT::Domain::GeometryDomain >::iterator it =
// dom_cache.find(id);
//            if (it != dom_cache.end())
//                return &(it->second);
//
...
//            }
//            dom_cache[id] = GVT::Domain::GeometryDomain(); // XXX TODO fix
// when this made a subclass
//
