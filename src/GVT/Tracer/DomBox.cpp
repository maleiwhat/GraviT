...
DomBox::DomBox(Material* mat, const Vector& p0, const Vector& p1,int domID)
  : PrimitiveCommon(mat), domID(domID)
{
  bbox[0] = Min(p0, p1);
  bbox[1] = Max(p0, p1);
  std::cout << "created dombox with bounds: " << bbox << std::endl;
}
...
void DomBox::setMinMax(const Vector&  p0, const Vector& p1)
{
  bbox[0] = Min(p0, p1);
  bbox[1] = Max(p0, p1);
}

void DomBox::computeBounds(const PreprocessContext&, BBox& bbox_) const
{
...
void DomBox::intersect(const RenderContext&, RayPacket& rays) const
{
  printf("intersected dombox!\n");
return;
...
void DomBox::computeNormal(const RenderContext&, RayPacket& rays) const
{
