#ifndef _MANTA_DOMBVH_H_
#define _MANTA_DOMBVH_H_
...
namespace Manta
{
  ...
  DomBVH : public AccelerationStructure,
           public AlignedAllocator<DomBVH, MAXCACHELINESIZE>
  {
  public:
    struct IAData
    {
  ...
  public:
    DomBVH(bool print = true) : subtreeListMutex("subtreeList"), subtreeListFilled(false),
                                num_nodes("DomBVH Num Nodes", 0), currGroup(NULL), mesh(NULL),
                                group_changed(false), barrier("DomBVH barrier"),
                                nextFree("DomBVH Next Free", 0), TaskListMemory(0),
    ...
    void intersect(const RenderContext& context, RayPacket& rays, std::vector<int>& domIDs) const;
  ...
  protected:
    void intersectNode(int nodeID, const RenderContext& context, RayPacket& rays, const IAData& ia_data, std::vector<int>& domIDs) const;
...
#ifdef MANTA_SSE
    // DomBVH ports
    void templatedTraverse(const RenderContext& context, RayPacket& packet,std::vector<int>& domIDs) const;
