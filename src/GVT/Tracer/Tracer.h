...
class abstract_trace {
 public:
  ...
  abstract_trace(GVT::Data::RayVector& rays, Image& image)
    ...
    this->colorBuf = new COLOR_ACCUM[GVT::Env::RTA::instance()->view.width *
                                     GVT::Env::RTA::instance()->view.height];
    this->queue_mutex =
        new boost::mutex[GVT::Env::RayTracerAttributes::rta->dataset->size()];
    this->colorBuf_mutex =
        new boost::mutex[GVT::Env::RTA::instance()->view.width];
  }
  ...
  virtual void addRay(GVT::Data::ray& ray) {
    ...
    if (!len2List.empty()) {
      ...
      GVT::Env::RayTracerAttributes::rta->dataset->getDomain(domTarget)
          ->marchIn(ray);
...
struct processRayVector {
  ...
  processRayVector(abstract_trace* tracer, GVT::Data::RayVector& rays,
        ...
        if (!len2List.empty()) {
          ...
          boost::mutex::scoped_lock fbloc(
              tracer->colorBuf_mutex
                  [ray.id % GVT::Env::RTA::instance()->view.width]);
          for (int i = 0; i < 3; i++)
...
class Tracer_base : public MPIW, public abstract_trace {
 public:
  Tracer_base(GVT::Data::RayVector& rays, Image& image)
      : MPIW(rays.size()), abstract_trace(rays, image) {}
  ...
  virtual void generateRays() {
    ...
    size_t workload = std::max(
        (size_t)1,
        (size_t)(this->rays.size() /
                 (GVT::Concurrency::asyncExec::instance()->numThreads * 2)));
    for (int rc = 0; rc < GVT::Concurrency::asyncExec::instance()->numThreads;
         ++rc) {
      GVT::Concurrency::asyncExec::instance()->run_task(processRayVector(
          this, this->rays, current_ray, this->rays_end, workload));
    }
  ...
  virtual void gatherFramebuffers(int rays_traced) {
    ...
    for (int i = 0; i < GVT::Env::RayTracerAttributes::rta->view.width *
                            GVT::Env::RayTracerAttributes::rta->view.height;
         ++i) {
    ...
    int rgb_buf_size = 3 * GVT::Env::RayTracerAttributes::rta->view.width *
                       GVT::Env::RayTracerAttributes::rta->view.height;

    unsigned char* bufs =
        (this->rank == 0) ? new unsigned char[this->world_size * rgb_buf_size]
                          : NULL;
    ...
    DEBUG(if (DEBUG_RANK) cerr << this->rank << ": rgb buffer merge done"
                               << endl);
