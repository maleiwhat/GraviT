...
void
DomBVH::printNode(int nodeID, int depth) const {
...
void DomBVH::intersect(const RenderContext& context, RayPacket& rays) const
{
  std::vector<int> domIDs;
  intersect(context,rays,domIDs);
}
    void DomBVH::intersect(const RenderContext& context, RayPacket& rays, std::vector<int>& domIDs) const
    {
...
#if USE_DomBVH_PORTS
  templatedTraverse(context, rays, domIDs);
#else
  rays.computeSigns();
  ...
  intersectNode(0,context,rays, ia_data, domIDs);
#endif // DomBVH port or not
}

void DomBVH::intersectNode(int nodeID, const RenderContext& context,
                           RayPacket& rays, const IAData& ia_data, std::vector<int>& domIDs) const
{
#if USE_LAZY_BUILD
    ...
    if (node.isLeaf()) {
      ...
      for (int i = 0; i < node.children; i++ ) {
        ...
        if (anyHit) {
          ...
          currGroup->get(object_id)->intersect(context, subpacket);
          DomBox* db = dynamic_cast<DomBox*>(currGroup->get(object_id));
          if (db)
          {
            domIDs.push_back(db->domID);
            printf("intersect dombox %d\n", db->domID);
          }
          else
            printf("intersect nondombox \n");
      ...
      int front_son = subpacket.getDirection(subpacket.begin(),
                                             static_cast<int>(node.axis)) > 0 ? 0 : 1;
#endif
      intersectNode(node.child+front_son, context, subpacket, ia_data,  domIDs);
      intersectNode(node.child+1-front_son, context, subpacket, ia_data,  domIDs);
    }
  }
}

// return the first index (between [rays.begin(),rays.end()]) which hits the box
int DomBVH::firstIntersects(const BBox& box, const RayPacket& rays, const IAData& ia_data)
{
...
int DomBVH::lastIntersects(const BBox& box, const RayPacket& rays)
{
  const bool anyHit = rays.getFlag(RayPacket::AnyHit);
...
void DomBVH::setGroup(Group* new_group) {
...
Group* DomBVH::getGroup() const {
  return currGroup;
}

void DomBVH::groupDirty()
{
  group_changed = true;
}

void DomBVH::addToUpdateGraph(ObjectUpdateGraph* graph,
...
void DomBVH::preprocess(const PreprocessContext& context)
{
  //cerr << MANTA_FUNC << endl;
  ...
  if (context.proc == 0 && context.isInitialized())
    context.manta_interface->registerParallelPreRenderCallback(
                                Callback::create(this, &DomBVH::updateStats));
#endif

}

typedef Callback_1Data_2Arg<DomBVH, Task*, int, UpdateContext> BVHUpdateTask;
typedef Callback_1Data_2Arg<DomBVH, TaskList*, int, Task*> BVHUpdateReduction;

typedef Callback_1Data_3Arg<DomBVH, Task*, int, int, UpdateContext> BVHPreprocessTask;

typedef Callback_1Data_4Arg<DomBVH, Task*, int, int, int, UpdateContext> BVHBuildTask;
typedef Callback_1Data_2Arg<DomBVH, TaskList*, int, Task*> BVHBuildReduction;
typedef Callback_1Data_5Arg<DomBVH, TaskList*, Task*, int, int, int, UpdateContext> BVHSAHReduction;


void DomBVH::beginParallelPreprocess(UpdateContext context) {
  ...
  new_list->setReduction(
    Callback::create(this,
                     &DomBVH::finishParallelPreprocess,
                     context));
...
void DomBVH::parallelPreprocess(Task* task, int objectBegin, int objectEnd, UpdateContext context) {
...
void DomBVH::finishParallelPreprocess(TaskList* tasklist, UpdateContext context) {
...
void DomBVH::beginParallelBuild(UpdateContext context) {
...
void DomBVH::splitBuild(Task* task, int nodeID, int objectBegin, int objectEnd, UpdateContext context) {
...
void DomBVH::parallelApproximateSAH(Task* task, int nodeID, int objectBegin, int objectEnd, UpdateContext context) {
  ...
  children->setReduction(new (FiveArgCallbackMemory + reduction_id * sizeof(BVHSAHReduction))BVHSAHReduction
                         (this,
                          &DomBVH::parallelComputeBoundsReduction,
                          task,
                          nodeID,
                          objectBegin,
                          objectEnd,
...
void DomBVH::parallelComputeBounds(Task* task, int nodeID, int objectBegin, int objectEnd, UpdateContext context) {
  BBox exact_bounds;
...
void DomBVH::parallelComputeBoundsReduction(TaskList* tasklist,
                                            Task* task,
                                            int nodeID,
                                            int objectBegin,
                                            int objectEnd,
  ...
  children->setReduction(new (FiveArgCallbackMemory + reduction_id * sizeof(BVHSAHReduction))BVHSAHReduction
                         (this,
                          &DomBVH::parallelComputeBinsReduction,
                          task,
                          nodeID,
                          objectBegin,
                          objectEnd,
...
void DomBVH::parallelComputeBins(Task* task, int nodeID, int objectBegin, int objectEnd, UpdateContext context) {
...
int DomBVH::partitionObjects(int objBegin, int objEnd, int axis, float position) const {
...
void DomBVH::parallelComputeBinsReduction(TaskList* tasklist,
                                          Task* task,
                                          int nodeID,
                                          int objectBegin,
                                          int objectEnd,
...
void DomBVH::parallelTopDownBuild(Task* task, int nodeID, int objectBegin, int objectEnd, UpdateContext context) {
...
void DomBVH::parallelBuildReduction(TaskList* list, int node_id, Task* task) {
  task->finished();
}

void DomBVH::update(int proc, int numProcs) {
...
void DomBVH::computeTraversalCost()
{
...
VectorT<float, 2> DomBVH::computeSubTreeTraversalCost(unsigned int nodeID)
{
...
void DomBVH::rebuild(int proc, int numProcs)
{
  if (!group_changed) {
  ...
  if (print_info && proc == 0)
    cerr << "\nDomBVH::preprocess START (" << currGroup->size() << " objects)\n";
#endif
  ...
  if (print_info) {
    cerr << "\nDomBVH build time: Total ("<<endTime-startTime<<")\n"
...
void DomBVH::build(int nodeID, int objectBegin, int objectEnd,
...
void DomBVH::lazyBuild(const RenderContext& context, const int nodeID) const
{
  BVHNode& node = nodes[nodeID];
...
unsigned int DomBVH::computeSubTreeSizes(size_t nodeID) {
...
void DomBVH::seedSubtrees(const int nodeID)
{
...
void DomBVH::parallelUpdateBounds(const PreprocessContext& context,
...
void DomBVH::updateBounds(int ID)
{
...
DomBVH::PartitionData DomBVH::partitionApproxSAH(int nodeID, int objBegin,
                                                   int objEnd) const {
  ...
  DomBVH::PartitionData DomBVH::partition2Objs(int nodeID, int objBegin) const {
    PartitionData partition;
  ...
  DomBVH::PartitionData DomBVH::partitionSAH(int nodeID,
...
bool DomBVH::buildEvents(const int parentID,
                         int first,
                         int last,
                         int axis,
...
bool DomBVH::buildFromFile(const string &file)
{
  nodes.clear();
...
bool DomBVH::saveToFile(const string &file)
{
  if (nodes.empty()) {
    needToSaveFile = true;
    saveFileName = file;
...
// begin port from actual DomBVH
#ifdef MANTA_SSE
void DomBVH::templatedTraverse(const RenderContext& context, RayPacket& packet, std::vector<int>& domIDs) const {
  struct StackNode {
    int nodeID;
    int firstActive;
  };
    ...
    for (;;) {
      ...
      if (firstActive < RayPacket::MaxSize) {
        if (!thisNode.isLeaf()) {
          ...
          // this part has to differ from DomBVH due to requirements
...
int DomBVH::firstActivePort(RayPacket& packet, int firstActive, const BBox& box) const {
...
int DomBVH::lastActivePort(RayPacket& packet, int firstActive, const BBox& box) const {
  __m128 box_min_x = _mm_set1_ps(box[0][0]);
  __m128 box_min_y = _mm_set1_ps(box[0][1]);
  __m128 box_min_z = _mm_set1_ps(box[0][2]);
  ...
  for (int i=last_ray; i > firstActive; i -= 4) {
    ...
    // Unlike DomBVH we want to return last active "ray", so the last
...
int DomBVH::firstActiveSameSignFrustumPort(RayPacket& packet,
...
int DomBVH::lastThatIntersectsSameSignPort(RayPacket& packet,
                                           const int firstActive,
                                           const BBox& box,
...
#endif // MANTA_SSE for USE_DomBVH_PORTS
...
void DomBVH::rotateNode(int const nodeID)
{
...
void DomBVH::rotateTree( int const nodeID )
{
  const BVHNode &node = nodes[ nodeID ];
  if ( !node.isLeaf() )
  {
...
Real DomBVH::computeCost(int const nodeID )
{
  const BVHNode &node = nodes[ nodeID ];
  Real cost;
  if ( node.isLeaf() ) {
...
namespace Manta {
  MANTA_DECLARE_RTTI_DERIVEDCLASS(DomBVH, AccelerationStructure, ConcreteClass, readwriteMethod);
  MANTA_REGISTER_CLASS(DomBVH);

#ifndef SWIG
  MANTA_DECLARE_RTTI_BASECLASS(DomBVH::BVHNode, ConcreteClass, readwriteMethod);
  MANTA_REGISTER_CLASS(DomBVH::BVHNode);
#endif
}

#ifndef SWIG
void DomBVH::BVHNode::readwrite(ArchiveElement* archive) {
...
void DomBVH::readwrite(ArchiveElement* archive) {
