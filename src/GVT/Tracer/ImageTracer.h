...
class Tracer<DomainType, MPIW, ImageSchedule> : public Tracer_base<MPIW> {
 public:
  Tracer(GVT::Data::RayVector& rays, Image& image)
    ...
    this->rays_end = (this->rank + 1) == this->world_size
                         ? this->rays.size()
                         : (this->rank + 1) * ray_portion;  // tack on any odd
                                                            // rays to last proc
  }
  ...
  virtual void operator()() {
    ...
    do {
      ...
      for (map<int, GVT::Data::RayVector>::iterator q = this->queue.begin();
           q != this->queue.end(); ++q) {
      ...
      GVT_DEBUG(DBG_ALWAYS, "Selecting new domain");
      if (domTarget != -1)
        std::cout << "Domain " << domTarget << " size "
                  << this->queue[domTarget].size() << std::endl;
      if (DEBUG_RANK)
        GVT_DEBUG(DBG_ALWAYS, this->rank << ": selected domain " << domTarget
                                         << " (" << domTargetCount << " rays)");
      if (DEBUG_RANK)
        GVT_DEBUG(DBG_ALWAYS, this->rank << ": currently processed "
                                         << ray_counter << " rays across "
      ...
      if (domTarget >= 0) {
        ...
        // domTarget, dom, this->colorBuf, ray_counter, domain_counter))();
        {
          ...
          boost::timer::auto_cpu_timer t(
              "ImageTracer: Tracing domain rays %t\n");
          dom->trace(this->queue[domTarget], moved_rays);
        }
        GVT_DEBUG(DBG_ALWAYS, "Marching rays");
        //                        BOOST_FOREACH( GVT::Data::ray* mr,
        // moved_rays) {
        //                            dom->marchOut(mr);
        //                            GVT::Concurrency::asyncExec::instance()
        //                            ->run_task(processRay(this,mr));
        //                        }
                ...
                (GVT::Concurrency::asyncExec::instance()->numThreads * 2)));
        {
          ...
          for (int rc = 0;
               rc < GVT::Concurrency::asyncExec::instance()->numThreads; ++rc) {
        ...
        // dom->free();
        moved_rays.clear();
        // this->queue.erase(domTarget); // TODO: for secondary rays, rays may
        // have been added to this domain queue
      }
    } while (domTarget != -1);
