...
class Camera {
 public:
  ...
  Camera(GVT::Data::RayVector& rays, GVT::Env::RayTracerAttributes::View& vi,
         float rate);

  void setEye(const GVT::Math::Vector4f& eye) { this->eye = eye; }
  ...
  void setLook(const GVT::Math::Vector4f& viewDir,
               const GVT::Math::Vector4f& upDir) {
    GVT::Math::Vector3f z = viewDir;  // this is where the z axis should end up
    z.normalize();
    GVT::Math::Vector3f y = upDir;  // where the y axis should end up
    y.normalize();
    GVT::Math::Vector3f x = y ^ z;  // lah,
    x.normalize();
    m = GVT::Math::AffineTransformMatrix<float>(
        x[0], x[1], x[2], 0.f, y[0], y[1], y[2], 0.f, z[0], z[1], z[2], 0.f,
        0.f, 0.f, 0.f, 1.f).transpose();
    update();
  }

  void setLook(GVT::Math::Vector4f& eyePos, GVT::Math::Vector4f& lookAt,
               const GVT::Math::Vector4f& upDir) {
    GVT::Math::Vector3f z =
        -(lookAt - eyePos)
  ...
  const GVT::Math::Vector4f& getEye() const { return eye; }

  const GVT::Math::Vector4f& getLook() const { return look; }

  const GVT::Math::Vector4f& getU() const { return u; }

  const GVT::Math::Vector4f& getV() const { return v; }

  const GVT::Math::AffineTransformMatrix<float> getMatrix() { return m; }
  ...
  void MakeCameraRays() {
    ...
    rays.reserve(nrays);
    int offset =
        vi.height / GVT::Concurrency::asyncExec::instance()->numThreads;
...

    {
      ...
      for (int start = 0; start < vi.height;) {
        ...
        GVT::Concurrency::asyncExec::instance()->run_task(
    ...
    GVT_DEBUG(DBG_ALWAYS,
              "EXPECTED PREGENERATING : " << (trcUpSampling * trcUpSampling) *
                                                 vi.width * vi.height);
        ...
        for (int i = 0; i < buffer_width; i++) {
          ...
          for (float off_i = 0; off_i < 1.0; off_i += offset) {
            for (float off_j = 0; off_j < 1.0; off_j += offset) {
              ...
              dir = (m * look).normalize();
              dir = dir + x * u.normalize();
              dir = dir + y * v.normalize();
...
template <>
Camera<C_ORTHOGRAPHIC>::Camera(GVT::Data::RayVector& rays,
                               GVT::Env::RayTracerAttributes::View& vi,
                               float rate);
template <>
Camera<C_PERSPECTIVE>::Camera(GVT::Data::RayVector& rays,
                              GVT::Env::RayTracerAttributes::View& vi,
                              float rate);
}
}
